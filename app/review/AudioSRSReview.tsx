      ...
      <ul className="w-full space-y-4">
        {queue.map((item) => (
          <li key={item.id} className="bg-white dark:bg-gray-800 rounded shadow p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link href={`/speak/word/${item.word_id}`} className="mt-2 sm:mt-0 inline-block bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700 transition-colors font-medium text-center">Practice</Link>
          </li>
        ))}
      </ul>
    </div>
